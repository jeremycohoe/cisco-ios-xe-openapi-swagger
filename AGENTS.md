...
A **second** live-data surface, distinct from the harness Live Data page: [device-data.html](device-data.html) / [device-data.js](device-data.js) compares the **same modules across 10 transports** (MDT, RESTCONF, NETCONF get/get-config/subscribe/subscribe-config, gNMI get/get-config/state/subscribe) for the 7 devices (the 6 lab boxes + the `.110` C9300-STACK8-WAN production WAN stack). It reads:

- `protocol-matrix.json` — the device x module x method status grid (`scripts/mdt-telemetry/collector/build_protocol_matrix.py`).
- **10 per-method browse datasets at the repo root**, one per transport button: `telemetry-live-data.json` (MDT), `restconf-live-data.json`, `netconf-{get,getconfig,sub}-live-data.json`, `netconf-sub-config-live-data.json`, `gnmi-{get,getconfig,state,sub}-live-data.json`. Each is device -> category -> path, with the page lazily fetching the per-path body only on drill-in.
